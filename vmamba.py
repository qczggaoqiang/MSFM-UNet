...
class MyDecoderLayer(nn.Module):
    def __init__(
            self, input_size, in_out_chan, head_count, token_mlp_mode, image_size, layer_depth=2, n_class=9,
...
class MEW(nn.Module):
    def __init__(self, dim, bias=False, a=16, b=16, c_h=16, c_w=16):
        ...
        # 将 dim 转换为张量，并将其作为缓冲区注册到模型中，名称为 "dim"，
        # 模型缓冲区通常是一些在训练和推理过程中需要保留的固定数据（比如常数），但它们不会被优化器更新。
        ...
        # InvertedDepthWiseConv2d 通常指的是一种基于深度可分离卷积的变体，常用于高效的卷积神经网络架构
    ...
    def forward(self, x):
        #print(x.shape)
        #conv1x1 = nn.Conv2d(in_channels=512, out_channels=128, kernel_size=1)
        #x = conv1x1(x).cuda()
        x1, x2, x3, x4 = torch.chunk(x, 4, dim=1)  # 沿通道方向将输入图像分割成4份
        B, c, a, b = x1.size()  # B 是批次大小，c 是通道数的四分之一，a 是高度，b 是宽度；
        # ----- a convlution -----#
        x1 = x1.permute(0, 2, 1, 3)  # B, a, c, b

        # 对张量沿着进行通道和宽度方向进行二维傅里叶变换
        x1 = torch.fft.rfft2(x1, dim=(2, 3), norm='ortho')
        a_weight = self.a_weight

        # 使用 F.interpolate 对 a_weight 进行双线性插值
        ...
        x1 = x1 * a_weight  # 加权
        # 使用 torch.fft.irfft2 进行逆傅里叶变换，将张量从频域还原到空间域
        # 并使用 permute 将张量维度顺序还原为 (B, c, a, b)，s=(c, b)表示输出尺寸
        ...
        c_weight = torch.view_as_complex(c_weight.contiguous()).cuda()
        x3 = x3 * c_weight
...
class Refine_Cross_Attention(nn.Module):
    def __init__(self, attention_prosition, img_size, in_dim, dim, mbconv_expansion_rate=4, mbconv_shrinkage_rate=0.25,
                 patch_size=3):
        ...
        self.conv0 = MBConv(96, dim, downsample=False, expansion_rate=mbconv_expansion_rate,
                            shrinkage_rate=mbconv_shrinkage_rate)
        self.conv0_1 = MBConv(192, dim, downsample=False, expansion_rate=mbconv_expansion_rate,
                              shrinkage_rate=mbconv_shrinkage_rate)
        self.conv20 = MBConv(192, dim, downsample=False, expansion_rate=mbconv_expansion_rate,
                             shrinkage_rate=mbconv_shrinkage_rate)
        self.conv30 = MBConv(384, dim, downsample=False, expansion_rate=mbconv_expansion_rate,
                             shrinkage_rate=mbconv_shrinkage_rate)
        self.conv40 = MBConv(128, dim, downsample=False, expansion_rate=mbconv_expansion_rate,
                             shrinkage_rate=mbconv_shrinkage_rate)
        self.conv1 = MBConv(dim, dim, downsample=False, expansion_rate=mbconv_expansion_rate,
                            shrinkage_rate=mbconv_shrinkage_rate)
        ...
        self.conv4 = MBConv(dim, dim, downsample=False, expansion_rate=mbconv_expansion_rate,
                            shrinkage_rate=mbconv_shrinkage_rate)
        self.conv1x1 = nn.Conv2d(in_channels=512, out_channels=128, kernel_size=1)
        self.mew = MEW(dim).cuda()
        # self.conv5 = MBConv(dim, dim, downsample=False, expansion_rate=mbconv_expansion_rate,
        #                     shrinkage_rate=mbconv_shrinkage_rate)
        if attention_prosition == 0:
            ...
            self.embedding1 = PatchMerging(img_size=img_size, patch_size=patch_size, stride=4, padding=1,
                                           in_ch=in_dim[0], dim=in_dim[1])
            self.embedding2 = PatchMerging(img_size=img_size, patch_size=patch_size, stride=2, padding=1,
                                           in_ch=in_dim[1], dim=in_dim[1])
            self.embedding3 = PatchExpand(input_resolution=(img_size // 32, img_size // 32), dim=in_dim[-1],
                                          dim_scale=2)

            self.linear_2 = nn.Linear(256, 320)
            self.linear_4 = nn.Linear(192, 128)
            self.linear_5 = nn.Linear(384, 128)
            self.channel_attn = ChannelAttention(dim)
            self.norm = nn.LayerNorm(dim)
            self.linear_downsample = nn.Linear(dim, in_dim[2], bias=False)

        if attention_prosition == 1:
            # From the bottom up to the first attention block, the input from top to bottom is twice downsampled, unchanged, and twice upsampled, respectively
            self.embedding1 = PatchMerging(img_size=img_size, patch_size=patch_size, stride=2, padding=1,
                                           in_ch=in_dim[0], dim=in_dim[1])
            self.embedding2 = PatchExpand(input_resolution=(img_size // 16, img_size // 16), dim=in_dim[2], dim_scale=2)
            self.embedding3 = FinalPatchExpand_X4(input_resolution=(img_size // 32, img_size // 32), dim=in_dim[3],
                                                  dim_scale=4)

            self.linear_2 = nn.Linear(256, 320)
            self.linear_4 = nn.Linear(192, 128)
            self.linear_5 = nn.Linear(768, 128)
            self.channel_attn = ChannelAttention(dim)
            self.norm = nn.LayerNorm(dim)
            self.linear_downsample = nn.Linear(dim, in_dim[1], bias=False)
        if attention_prosition == 2:
            ...
            self.embedding2 = FinalPatchExpand_X4(input_resolution=(img_size // 16, img_size // 16), dim=in_dim[2],
                                                  dim_scale=4)
            self.embedding3 = FinalPatchExpand_X8(input_resolution=(img_size // 32, img_size // 32), dim=in_dim[3],
                                                  dim_scale=8)
            self.linear_2 = nn.Linear(96, 128)
            self.linear_3 = nn.Linear(384, 128)
            self.linear_4 = nn.Linear(768, 128)
            self.channel_attn = ChannelAttention(dim)
            self.norm = nn.LayerNorm(dim)
            self.linear_downsample = nn.Linear(dim, in_dim[0], bias=False)

    def forward(self, x):
        if self.attention_prosition == 0:
            embed1 = x[0]
            embed1, H, W = self.embedding1(embed1)
            embed1 = Rearrange("b (h w) d -> b d h w", h=H, w=W)(embed1)

            embed2 = x[1]
            embed2, H, W = self.embedding2(embed2)
            embed2 = Rearrange("b (h w) d -> b d h w", h=H, w=W)(embed2)

            embed3 = x[2]

            embed4 = x[3]
            embed4 = self.embedding3(embed4)
            embed4 = Rearrange("b d h w -> b (h w) d", h=H, w=W)(embed4)
            embed4 = self.linear_5(embed4)
            embed4 = Rearrange("b (h w) d -> b d h w", h=H, w=W)(embed4)
        ...
        if self.attention_prosition == 2:
            ...
            embed4 = self.embedding3(embed4)
            embed4 = Rearrange("b d h w -> b (h w) d", h=H, w=W)(embed4)
            embed4 = self.linear_4(embed4)
            embed4 = Rearrange("b (h w) d -> b d h w", h=H, w=W)(embed4)

        # MBConv is used as the relative position encoding
        if embed1.shape[1] == 96:
            embed1 = self.conv0(embed1)
        elif embed1.shape[1] == 192:
            embed1 = self.conv0_1(embed1)
        embed1 = self.conv1(embed1)
        if embed2.shape[1] == 192:
            embed2 = self.conv20(embed2)
        embed2 = self.conv2(embed2)
        if embed3.shape[1] == 384:
            embed3 = self.conv30(embed3)
        embed3 = self.conv3(embed3)
        if embed4.shape[1] == 192:
            embed4 = self.conv40(embed4)
        embed4 = self.conv4(embed4)

        embed = torch.cat([embed1, embed2, embed3, embed4], dim=1)
        #print(embed.shape)
        embed=self.conv1x1(embed)
        embed = self.mew(embed)
        #print(embed.shape)
        embed = Rearrange("b d h w -> b (h w) d", h=H, w=W)(embed)
        #print(embed.shape)
        chan_embed = self.norm(embed)
        #print(chan_embed.shape)
        #print(chan_embed.shape)
...
class VSSM(nn.Module):
    def __init__(self, patch_size=4, in_chans=3, num_classes=1000, depths=[2, 2, 9, 2], depths_decoder=[2, 9, 2, 2],
        ...
        self.refine_attn_3 = Refine_Cross_Attention(attention_prosition=2, img_size=256, in_dim=[96, 192, 384, 768],
                                                    dim=128)
        self.refine_attn_2 = Refine_Cross_Attention(attention_prosition=1, img_size=256, in_dim=[96, 192, 384, 768],
                                                    dim=128)
        self.refine_attn_1 = Refine_Cross_Attention(attention_prosition=0, img_size=256, in_dim=[96, 192, 384, 768],
                                                    dim=128)
    ...
    def forward(self, x):
        ...
        skip_list = [feature.permute(0, 3, 1, 2) for feature in skip_list]
        refine_attn_3 = self.refine_attn_3(skip_list)
        skip_list[0] = (Rearrange(" b (h w) d -> b d h w", h=64, w=64)(refine_attn_3))
        refine_attn_2 = self.refine_attn_2(skip_list)
        skip_list[1] = Rearrange(" b (h w) d -> b d h w", h=32, w=32)(refine_attn_2)
        refine_attn_1 = self.refine_attn_1(skip_list)
        skip_list[2] = Rearrange(" b (h w) d -> b d h w", h=16, w=16)(refine_attn_1)
        skip_list = [feature.permute(0, 2, 3, 1) for feature in skip_list]
